...
// Global state
//
static const uint256 hashGenesisBlockOfficial("0x00000f9aecc1635311c92e09c473384c7356b38762cf2ece0137d8c89952f09b");
static const uint256 hashGenesisBlockTestNet ("0x0000008991adb32e4bf4b583040cc469065ef6eff84f43edd78dbd7361e348a6");
...
const string strMessageMagic = "FTCoin Signed Message:\n";
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // FTCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // FTCoin: enforce minimum output amount
...
// FTCoin: minter's coin stake is rewarded based on coin age spent (coin-days)
...
static const int64 nTargetTimespan = 16 * 60 * 60;  // 16 mins
...
bool CBlock::AcceptBlock()
{
    ...
    // FTCoin: check switch from proof of work to proof of stake
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "FTCoin", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "FTCoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Created in China Date:2015-08-25 - build is for public";
        unsigned int nTimeGenesis=1440482399;
        unsigned int nNonceGenesis=1884550;

        if (fTestNet)
        {
            pszTimestamp="Created in China Date:2015-08-25 - build is for public test!";
            nTimeGenesis=1440482399;
            nNonceGenesis=1575515;
        }
        ...
        printf("FTCoin Genesis Block Found:\n");
        printf("genesis hash=%s\n", block.GetHash().ToString().c_str());
        printf("merkle root=%s\n", block.hashMerkleRoot.ToString().c_str());
        block.print();
     
        printf("End FTCoin Genesis Block\n");
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        if (!fTestNet)
            assert(block.hashMerkleRoot == uint256("0x9d112c0491dd9318717c75b8ecb586668d8c0ce871307c9bf59312c8907f2bbe"));
        else
            assert(block.hashMerkleRoot == uint256("0x8928134bf2bff53ae78504d333962672f0a62f9ee86ab1f95cfe027e4cf8cb0a"));
...
string GetWarnings(string strFor)
{
    ...
    {
        nPriority = 3000;
        strStatusBar = strRPC = "WARNING: Invalid checkpoint found! Displayed transactions may not be correct! You may need to upgrade, or notify developers of the issue (https://github.com/FTCoin/FTCoin/issues).";
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave)
                pfrom->AskFor(inv, IsInitialBlockDownload()); // FTCoin: immediate retry during initial download
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        loop
        {
            ...
            printf("Check if something found nNonceFound=%d",nNonceFound);
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        int nAddThreads = (/*fTestNet?*/ nProcessors /*: 1*/) - vnThreadsRunning[THREAD_MINER];
