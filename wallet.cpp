...
bool CWallet::AddKey(const CKey& key)
{
    ...
    if (!fFileBacked)
        return true;
    if (!IsCrypted())
        return CWalletDB(strCommonWalletFile).WriteKey(key.GetPubKey(), key.GetPrivKey());
    return true;
}
...
bool CWallet::AddCryptedKey(const vector<unsigned char> &vchPubKey, const vector<unsigned char> &vchCryptedSecret)
{
        ...
        return true;
    {
        ...
        if (pwalletdbEncryption)
            ...
            return CWalletDB(strCommonWalletFile).WriteCryptedKey(vchPubKey, vchCryptedSecret);
    }
    return false;
}
        ...
        BOOST_FOREACH(MasterKeyMap::value_type& pMasterKey, mapMasterKeys)
        {
            ...
            if (CCryptoKeyStore::Unlock(vMasterKey))
            {
                ...
                if (!crypter.Encrypt(vMasterKey, pMasterKey.second.vchCryptedKey))
                    return false;
                CWalletDB(strCommonWalletFile).WriteMasterKey(pMasterKey.first, pMasterKey.second);
                if (fWasLocked)
                    Lock();
                return true;
            }
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        if (fFileBacked)
        {
            pwalletdbEncryption = new CWalletDB(strCommonWalletFile);
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    if (nTotalLower == nTargetValue || nTotalLower == nTargetValue + uCOIN)
    {
    ...
    if (nTotalLower < nTargetValue + (coinLowestLarger.second.first ? uCOIN : 0))
    {
    ...
    if (nTotalLower >= nTargetValue + uCOIN)
        nTargetValue += uCOIN;
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                if (nFeeRet < MIN_TX_FEE && nChange > 0 && nChange < uCOIN)
                {
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    loop
    {
        ...
        {
            txNew.vout[1].nValue = ((nCredit - nMinFee) / 2 / uCOIN) * uCOIN;
...
int CWallet::LoadWallet(bool& fFirstRunRet)
{
    if (!fFileBacked)
        return false;
    fFirstRunRet = false;
//    filesystem::path commenPath = GetDataDir(false) / ".." / "common";
//    if(GetBoolArg("-testnet", false))
//        commenPath /= "testnet";
//    filesystem::create_directories(commenPath);
    int nLoadWalletRet = CWalletDB(strWalletFile,"cr+").LoadWallet(this);
    if(strCommonWalletFile != strWalletFile)
        nLoadWalletRet = CWalletDB(strCommonWalletFile.c_str(),"cr+").LoadWallet(this);
...
bool CWallet::NewKeyPool()
{
    {
        LOCK(cs_wallet);
        CWalletDB walletdb(strCommonWalletFile);
